...
The frontend Image Viewers settings UI manages registered and managed environments, defaults, ordered filename rules, lifecycle operations, and empty launches. Passive viewing checks still guide workflow import and prefill managed setup, but the per-output readiness report is not shown in Settings. Forgetting an adopted legacy installation suppresses its automatic startup adoption; changed-interpreter entries show the prior inventory as last checked and external entries can be located again.
