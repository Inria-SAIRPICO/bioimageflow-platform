...
Every direct workflow identity change, whole-folder rename, and folder-child promotion is protected by one exclusive, operation-specific workspace move journal.
The prepared record is durably written before the generation ledger or any workflow, managed-storage, draft, metadata, retained-snapshot, or folder topology mutation and captures the exact source/destination paths, affected identity generations, intended metadata, managed-storage transition, and promotion-child inventory.
The exact generation reservation is the forward-completion commit point: an untouched prepared operation with all generations still at their recorded before values is abandoned, while a committed or partially applied operation is completed forward without incrementing a generation twice.
The durable phases are `prepared`, `artifacts_rewritten`, and `snapshots_rewritten`; file replacements and source/destination directory entries are flushed before their phase advances, and the journal is removed only after retained snapshot ownership is durable.
Recovery is idempotent and fails closed rather than guessing when the journal is malformed, generations are mixed, source and destination both exist or are both missing, managed-storage authority differs, a folder promotion has unrecorded children, or retained snapshots are unreadable.
Application startup completes or abandons pending moves before orphan snapshot cleanup, workflow custom-tool discovery, package catalog refresh, and request serving; an ambiguous recovery is a fatal startup error.
An interrupted live request that leaves a committed journal returns `503 workflow_move_recovery_required`, while workflow-tree notification publication after a durable commit is best-effort and cannot turn that commit into a failed mutation.
While any move journal remains, the server serializes and rejects every later mutating API request and every supported store, draft, or nested-snapshot write so tool creation, execution output, autosave, deletion, or an unrelated workflow mutation cannot invalidate recovery.
The identity-moving store executors require the exact prepared operation ID and revalidate its source, destination, topology, generations, managed storage, and target metadata before crossing the generation commit point; direct callers cannot bypass the journal protocol.
...
Parameter edits synchronously apply the operational semantic invalidation required by the edited graph, while the presented node status retains its meaningful label and color or transitions once to the correct post-edit execution state instead of showing a gray or `provisional` persistence state.
Each canonical canvas session owns one explicit persistence presentation state—`idle`, `saving`, `error`, or `conflict`—and reopening or activating that workflow reuses the same authority.
Ordinary persistence shows only a quiet, non-blocking `Saving…` indicator and returns silently to `idle` after successful acceptance, without a success banner.
Save errors and conflicts remain readable and actionable until resolved or dismissed, support keyboard focus and pause-on-hover, and are announced through an appropriate live region; rapid edits coalesce deterministically around the latest snapshot, remain `saving` until that snapshot is accepted, and ignore superseded completions without semantic-color flicker or transient notices.
A frontend-authored draft WebSocket event received while that canvas is awaiting its local draft acceptance is treated as a pending transport echo and does not flash the external-conflict banner; matching HTTP acceptance clears it, while a rejected or revision-conflicting request promotes the condition to the durable conflict state.
