...
test('napari result action resolves the captured identity to the server-selected environment', async ({ page }) => {
  ...
  await page.route('**/api/v1/data-table/query', async route => {
    await route.fulfill({ json: {
      ...
      columns: [{ id: 's0:mask', label: 'mask', type: 'ImageFile', source_node_id: 'seed', source_column: 'mask' }],
      rows: [{ index: 'selected', values: { 's0:mask': '/captured/labels.ome.tif' }, source_rows: { seed: 42 } }],
      total_rows: 1,
