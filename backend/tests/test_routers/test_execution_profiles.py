...
async def test_profile_crud_and_targets(profile_client: httpx.AsyncClient) -> None:
    ...
    patch = {
        "expected_revision": 1,
        "profile": profile_fields(name="Renamed").model_dump(mode="json"),
    }
...
async def test_profile_test_uses_public_validation(
    profile_client: httpx.AsyncClient, monkeypatch: pytest.MonkeyPatch
) -> None:
    created = (
        await profile_client.post(
            "/api/v1/execution/profiles",
    ...
    response = await profile_client.post(f"/api/v1/execution/profiles/{created['id']}/test")
