...
def get_openhands_service() -> OpenHandsService:  # pragma: no cover
    raise RuntimeError("openhands_service dependency not configured")
...
def _ensure_available(service: OpenHandsService) -> None:
    ...
    raise HTTPException(
        ...
        detail={
            ...
            "detail": context.reason or "OpenHands is unavailable",
        },
    )
...
async def get_agent_context(
    ...
    openhands_service: OpenHandsService = Depends(get_openhands_service),
) -> AgentContextResponse:
    _ensure_available(openhands_service)
...
@router.get(
    "/package-install-requests",
    response_model=list[AgentPackageInstallRequestResponse],
)
async def list_package_install_requests(
    service: AgentWorkspaceService = Depends(get_agent_workspace_service),
    openhands_service: OpenHandsService = Depends(get_openhands_service),
) -> list[AgentPackageInstallRequestResponse]:
    _ensure_available(openhands_service)
    return [
        AgentPackageInstallRequestResponse(
            id=request.id,
        ...
        for request in service.list_package_install_requests()
    ]
