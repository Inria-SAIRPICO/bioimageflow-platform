...
| T04 | In progress | Required hot-reload fixture enforcement is committed; shared-downstream cache coverage now uses a valid graph, and cache-parity/output-template fixtures are explicitly sequential. Remaining priority gaps include a valid recovery graph and actual ToolsPanel click. | Commit the validated runtime test repairs, then implement the two remaining bounded gaps. |
