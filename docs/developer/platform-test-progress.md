...
1. Complete the active Wave 4 recursive owned-source copy/transport and execution-ownership batch.
2. Close the remaining Wave 3 root discard/recovery and bounded persistence obligations.
3. Begin Wave 5 failure, cancellation, correction/rerun, and remaining cache/result lifecycle journeys, then continue the ordered feature waves without dropping lower-priority coverage.
...
| Wave 3 / editing | In progress | Commits through `30faeb6` verify node state, connection/history, clipboard identity/structure, active-root shortcuts, text-entry focus safety, typed parameters, Shift-drag box selection, atomic bulk Delete with Undo/Redo, and accepted root output Clear/refusal. Root discard/recovery and bounded persistence gaps remain. |
| Wave 4 / nested workflows | In progress | Commits through `fd96f8f` verify private Apply/discard, stale-parent refusal, destructive port confirmation, forwarded-interface reconciliation, failed-write retry with unrelated parent edits, concurrent newer private state, remount dirtiness, and real nested Direct execution with scoped durable results in both browsers. Recursive source ownership and later recovery boundaries remain. |
...
`/root/root_discard_recovery` owns `.worktrees/root-discard-recovery` on `campaign/root-discard-recovery`.
