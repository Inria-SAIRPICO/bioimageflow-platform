...
2. Complete the active Wave 3 parameter-control journey for enum, boolean, list error/recovery, default/reset, connected-field behavior, and persistence.
3. Close Wave 3 bulk Delete/Clear and remaining selection/persistence gaps, then continue the ordered feature waves without dropping lower-priority coverage.
...
| Wave 3 / editing | In progress | Commits through `790345a` verify node state, connection/history, clipboard identity/structure, active-root shortcuts, and text-entry focus safety. A dedicated Sol/medium worktree owns the remaining parameter-control matrix; bulk Delete/Clear and bounded selection/persistence gaps remain. |
...
`/root/parameter_controls` owns `.worktrees/parameter-controls` on `campaign/parameter-controls`.
