...
1. Rerun the integrated Chromium/Firefox browser milestone checks after the [ISSUE-016](platform-test-issues.md) startup-selection fix; all Tier 1B primary-GUI cells have evidence, but the prior milestone lane was red.
2. Investigate the still-open [ISSUE-014](platform-test-issues.md) intermittent workflow-title symptom if it remains after the startup fix; preserve the observed BioImageFlow 0.7.2 sequential row-progress timing as a documented limitation.
