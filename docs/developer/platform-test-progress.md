...
2. Run the Wave 5 headed confidence checkpoint.
3. Begin Wave 6 with the highest-priority remaining GUI obligations.
...
| T03 systematic inventory | Provisional dashboard under correction | The [obligation dashboard](platform-test-dashboard.md) maps all 98 inventory rows, but a GPT-6 Astra/high audit found that applicability, blocker, primary-GUI, and source-row traceability must be corrected before its percentages are canonical. The `01ddbd9` arithmetic is reproducible but provisional; active image/file/export evidence is not yet credited. |
...
`/root/dashboard_correction` owns `.worktrees/dashboard-correction` on `campaign/dashboard-correction` for the full applicability audit, stable inventory IDs, blocker/primary reconciliation, and mechanical dashboard verifier.
