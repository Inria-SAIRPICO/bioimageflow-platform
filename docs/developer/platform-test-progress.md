...
1. Resolve [ISSUE-016](platform-test-issues.md) startup-selection precedence, then rerun invalidated integrated browser milestone checks; all Tier 1B primary-GUI cells have evidence, but the milestone lane is red.
...
The integrated `scripts/test check browser-all` at `0ab6bac` started both browser projects with the 240-second backend allowance but failed after 636s: Firefox's agent-copy reload opened the prior workflow; Chromium passed 62 cases, including both new Tier 1B journeys, then recursive Run Selected reload opened an older clipboard workflow (41 cases unrun). The browser failures expose ISSUE-016's startup-precedence contract mismatch; a runner-level Chromium error also appeared after the failed selector. The initial sandbox attempt could not allocate local ports before collection; the elevated retry reached tests. No full-browser pass is claimed, and the affected startup work is assigned to a dedicated worktree.
