...
2. Receive the Astra/high disposition for ISSUE-006, then resume or revise the frozen nested stale-parent repair strictly within that boundary.
...
| Wave 4 / nested workflows | Astra review | Commit `af58e4e` verifies the primary private-edit journey; ISSUE-006 freezes the proposed stale-parent resolution pending a bounded Astra/high review, and destructive removal has not started. |
...
`/root/nested_conflict_astra` owns the read-only ISSUE-006 review of that worktree; no stale-conflict implementation commit is authorized before disposition.
