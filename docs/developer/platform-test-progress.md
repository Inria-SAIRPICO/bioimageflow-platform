...
`/root/issue_005_fix` owns `.worktrees/issue_005_fix` on `campaign/issue-005-fix` within the completed Astra boundary; `/root/main_gui_journey` owns `.worktrees/main_gui_journey` on `campaign/main-gui-journey` for the independent Wave 2 browser batch; `/root/everyday_node_editing` owns `.worktrees/everyday_node_editing` on `campaign/everyday-node-editing` for the bounded Wave 3 node-state batch.
