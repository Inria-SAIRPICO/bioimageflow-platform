...
The dashboard baseline is `b213223` (2026-09-15).
...
`Primary GUI` identifies obligations needed for the ordinary create/edit/connect/run/inspect/save/reopen journeys and their immediate loss-prevention and result-lifecycle boundaries.
...
The numeric suffix is the row ordinal in that source table at this baseline, which gives every entry an explicit source-row reference; inserting or reordering source prose must not renumber an existing ID.
...
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
| GUI-SHL-001 | Default docks, menu, and panel visibility | P1 | Desktop and webapp GUI | no | G | G | G | G | G | gap | `fcb4408` inspection |
...
| GUI-SHL-004 | Recovered-edge geometry | P0 | Desktop and webapp GUI | yes | V | G | G | V | G | in-progress | `dd20821` |
| GUI-SHL-005 | Preferences opening and desktop availability | P1 | Desktop and webapp GUI | no | G | G | G | G | G | gap | `fcb4408` inspection |
| GUI-SHL-006 | Persisted Node Data page size | P1 | Desktop and webapp GUI/settings API | no | G | G | G | G | U | unassessed | `fcb4408` inspection |
...
| GUI-EDT-002 | Fit large/small graphs and encoded paths | P1 | Desktop and webapp GUI | no | G | G | G | G | G | gap | `fcb4408` inspection |
...
| GUI-EDT-005 | Mapped/collective column appearance | P1 | Desktop and webapp GUI | no | G | G | G | G | G | gap | `fcb4408` inspection |
...
| GUI-EDT-007 | Node rename preserves identity and refuses duplicates | P0 | Desktop and webapp GUI | yes | V | G | N (synchronous; no cancel/recovery lifecycle) | V | G | in-progress | `ec0ca3b` |
...
| GUI-AUT-001 | Catalog search and selected-tool reveal | P1 | Desktop and webapp GUI | no | G | G | G | G | G | gap | `fcb4408` inspection |
...
| GUI-AUT-005 | Embedded editor startup, progress, and stable iframe | P2 | Desktop GUI/external editor | no | G | G | B | G | B | blocked | `fcb4408` inspection |
| GUI-AUT-006 | Node-addressed source opening | P1 | Desktop GUI/API | no | G | G | G | G | G | gap | `fcb4408` inspection |
...
| Stable ID | Feature (inventory row) | Priority | Supported surface | Primary GUI | S | R | F | P | B | Status | Evidence revision |
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
| WF-LIF-001 | Canonical saved document and graph-owned visible name | P1 | Workflow API/storage | no | G | G | G | G | G | gap | `fcb4408` inspection |
...
| WF-LIF-007 | Reject stale duplicate generation | P1 | Workflow API/storage | no | G | G | G | G | G | gap | `fcb4408` inspection |
| WF-LIF-008 | Durable workflow generation across restart | P1 | Workflow API/storage | no | G | G | G | G | G | gap | `fcb4408` inspection |
...
| WF-REC-002 | Positional DataFrame publication and compaction | P1 | Desktop and webapp nested GUI/API | no | G | G | G | G | G | gap | `fcb4408` inspection |
| WF-REC-003 | Forward a child DataFrame port through parent interface | P1 | Desktop and webapp nested GUI/API | no | V | G | G | V | V | in-progress | `3422e1c` |
| WF-REC-004 | Deleting exposed node prunes interface atomically | P1 | Desktop and webapp GUI/API | no | G | G | G | G | G | gap | `fcb4408` inspection |
...
| WF-REC-008 | Nested CAS and descendant cleanup | P1 | Nested API/storage | no | G | G | G | G | G | gap | `fcb4408` inspection |
| WF-REC-009 | Nested conflict waits for explicit choice | P1 | Nested GUI/session coordinator | no | G | G | G | G | G | gap | `fcb4408` inspection |
| WF-REC-010 | Stale parent refuses nested apply | P0 | Desktop and webapp nested GUI/API | yes | V | V | V | V | G | in-progress | `f920c9b` |
| WF-REC-011 | Destructive nested interface apply is confirmed and recoverable | P0 | Desktop and webapp nested GUI/API | yes | V | V | G | V | G | in-progress | `cf794fb` |
...
| WF-REC-014 | Containment-cycle rejection | P1 | Workflow API/storage | no | G | G | G | G | G | gap | `fcb4408` inspection |
| WF-SRC-001 | Explicit identity-bound source update preview/apply | P1 | Desktop API/storage | no | G | V | V | V | V | gap | `d02dea9` |
...
| RT-EXE-012 | Latest projection symlink/pointer fallback | P2 | Local filesystem/runtime | no | G | G | G | G | U | unassessed | `fcb4408` inspection |
...
| RT-RES-002 | Merge compatible selections and stack unrelated data | P0 | Desktop and webapp GUI/API | yes | V | G | G | V | V | in-progress | `c050207` |
...
| RT-RES-008 | Thumbnail placeholder, paths, formats, and GUI display | P1 | Desktop and webapp GUI/API | no | G | G | B | G | G | blocked | `fcb4408` inspection |
| RT-RES-009 | Reveal path and native file dialogs | P2 | Native desktop/manual | no | G | G | B | G | B | blocked | `fcb4408` inspection |
| RT-RES-010 | Fiji result resolution and native launch | P2 | Native desktop/manual external app | no | G | G | B | N (launch is ephemeral; settings persistence is GUI-SHL-008) | B | blocked | `fcb4408` inspection |
