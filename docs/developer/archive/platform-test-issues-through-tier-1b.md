...
# Archived platform test campaign issues through Tier 1B
...
Status: resolved by `ac165b4` with a dedicated Sol/medium worktree; integrated broad browser validation remains pending.
...
The isolated pre-fix Firefox selector passed, so that broad-lane symptom was timing-dependent, while the focused unit regression failed deterministically. The fix removes the cross-workflow recovery candidate: valid last-opened wins, otherwise stable workflow order, and a newer recovery still applies to the chosen workflow only. The copy E2E now waits for durable last-opened preference and plants a newer recovery for the original before reload, making the ownership boundary deterministic.
On patch-equivalent task content, the focused startup tests passed 8/8 after the repair, exact Firefox and Chromium copy journeys passed 1/1 each, `scripts/test check frontend` passed 1,334 tests with lint/type/build, and cross-browser smoke passed 18/18 in each browser. The existing v1 §4.3 and `PLATFORM_CONTEXT.md` already stated this corrected order; no spec change was needed. The original integrated browser-all run remains red until rerun on `ac165b4` or later. This issue is not a result-table width defect; that test explicitly reopened its intended workflow after reload.
