...
- ISSUE-005 specialist assessment: GPT-6 Astra/high reproduced the manager failure read-only at `b6072f5` with installed BioImageFlow 0.7.1 in 6.02s. The manager rejected two `missing_tool` diagnostics scoped only to the disconnected invalid node, while public selected-node computation returned exact rows `(1, one, 2)`, `(2, two, 3)`, and `(3, three, 4)` from the intact full graph. The disposition is `safe-to-fix`; no files, library code, or runtime contracts were changed by the assessment, and implementation validation remains outstanding.
